...
2. **Per-process state isolation during cutover.** Locked: each backend keeps its own segment dir AND its own SQLite database. Bun → `tmp/segments/` + `tmp/xstream.db`; Rust → `tmp/segments-rust/` + `tmp/xstream-rust.db`. Implemented in `AppConfig::dev_defaults` at `server-rust/src/config.rs` and the Rust dev script's `DB_PATH` env var. The two backends are runtime-independent — neither knows about the other's job_store, segment cache, or DB rows — and sharing the DB during testing produced cross-contamination (Bun resolved transcode_jobs rows written by Rust whose `segment_dir` pointed at Rust's filesystem, then served 0 segments on a 200 OK). At Step 3 Bun is removed and the Rust DB becomes THE DB; until then, Rust seeds its DB by copying from Bun's once and the two diverge from there. See `Plan/Open-Questions.md §10.4`.
