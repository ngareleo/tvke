...
- **CI release matrix.** `.github/workflows/release.yml` triggered on tag push (`v*.*.*`). This workflow is introduced in unsigned, Linux-only form during Step 3 (a draft pre-release verification mechanism — see [Step 3 boundary note](03-Tauri-Packaging.md)). Step 4 extends it: native runner per OS — no cross-compile. Per-job sequence: checkout → build server + client → setup-ffmpeg → `tauri build --target <triple>` → sign → upload artifact → publish updater manifest. The `ci.yml` `tauri-build` job (also introduced in Step 3) stays as the per-PR build-parity check.
