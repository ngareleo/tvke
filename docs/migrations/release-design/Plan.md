...
- **`<SeasonsPanel>` prop contract** — shipped in M4 (commit `60b6ff7`);
  extended in M5 with Relay fragments + consumers (DetailPane, FilmRow
  inline expansion, FilmDetailsOverlay). M7 wires the final consumer
  (PlayerSidebar, series episode picker with `accordion={true}`). Props:
  `seasons`, `defaultOpenFirst`, `activeEpisode`, `onSelectEpisode`,
  `accordion`. Per-episode `watched/progressSeconds` rendering deferred
  (schema fields not yet exposed).
