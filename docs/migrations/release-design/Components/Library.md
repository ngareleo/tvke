...
6. **Actions row (`.overlayActions`):** `display: flex`, `alignItems: center`, `columnGap: 20px`, `marginTop: 8px`. Contains:
   - **Play CTA (`.playCta`):** `<button onClick={playWithTransition}>` — **glass pill** with **green "lighted sign" hover effect**. See [`FilmDetailsOverlay.md`](FilmDetailsOverlay.md#play-cta-glass-pill-liquid-glass-with-green-neon-sign-hover) for the complete spec (at-rest styling, hover "lighted sign" effect with oklch green tints, alpha-gradient borders, text-shadow and box-shadow glows, engraved icon treatment). Contains `<IconPlay>` + `<span>Play</span>`. `playWithTransition` uses `document.startViewTransition(() => navigate("/player/{film.id}"))` when available, else plain `navigate(...)`.
...
- [x] `.page`: flex column, `height: 100%`, `overflowX: hidden`, `overflowY: auto`, `backgroundColor: colorBg0`, **`paddingLeft: 40px`**, **`paddingRight: 40px`** (no top padding — hero starts at y=0 inside the padded container)
...
- [x] Production implementation (M4, 2026-05-03). LibraryPage shell with flex column layout, 75vh inset rounded hero, three-row carousel (Continue Watching + New Releases + Watchlist), search bar with custom green caret, FilterSlide TUI panel, FilmDetailsOverlay with glass Play CTA, `pickSuggestions` helper, full-bleed overlay at `?film=<id>`. All child components extracted and landed: `SearchSlide`, `FilterSlide`, `PosterRow`, `FilmTile`, `FilmDetailsOverlay`, `SeasonsPanel`. View-transition contract synced with Player `.backdrop` via `viewTransitionName: "film-backdrop"`. Mock data replaced with Relay `LibraryPageContentQuery` (videos + watchlist). RAF-eased row scroll with pagination arrows, grid responsive to viewport. TODO items (hero slideshow rotation, greeting 3D tilt, mouse proximity tracking) deferred to polish pass.
