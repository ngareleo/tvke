 ...
 *
 * Only active in development (import.meta.env.DEV). In production the provider
 * is replaced with a no-op so zero runtime overhead ships to users.
 *
 * Usage:
 *   // In any component you want to be throwable:
 *   <DevThrowTarget id="Dashboard">
 *     <Dashboard />
 *   </DevThrowTarget>
 *
 *   // The DevPanel lists all registered targets. Clicking "⚡ Throw" on one
...
export const DevThrowTarget: FC<{ id: string; children: ReactNode }> = ({ id, children }) => {
  ...
  if (throwTarget === id) {
    // Reset the target first so retry works cleanly, then throw
    setThrowTarget(null);
    throw new Error(
